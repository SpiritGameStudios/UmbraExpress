...
public class TimeRendererMixin {
	...
	private static boolean mysticCooldown(boolean original, @Share("isMystic")LocalBooleanRef localBooleanRef, @Local(argsOnly = true) ClientPlayerEntity player, @Local GameWorldComponent game) {
		boolean mystic = game.isRole(player, UmbraExpressRoles.MYSTIC) && CrystalBallWorldComponent.KEY.get(player.getWorld()).isOnCooldown();
	...
	private static int replaceTimeWithMystic(GameTimeComponent instance, Operation<Integer> original, @Local(argsOnly = true) ClientPlayerEntity player, @Share("isMystic") LocalBooleanRef localBooleanRef) {
