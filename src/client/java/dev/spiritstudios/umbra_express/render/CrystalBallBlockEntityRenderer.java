...
import static dev.spiritstudios.umbra_express.block.entity.CrystalBallBlockEntity.MAX_APPARITION_TICKS;

public class CrystalBallBlockEntityRenderer implements BlockEntityRenderer<CrystalBallBlockEntity> {

    private static final float HEAD_PIXEL_WIDTH = 6;
    private static final int BALL_WIDTH = 10;
    private static final int BASE_HEIGHT = 3;

    private static final float HEAD_SCALE = 1 / 8F * HEAD_PIXEL_WIDTH;
    private static final float HEAD_AND_BALL_SCALE_DIFF = (BALL_WIDTH - HEAD_PIXEL_WIDTH) / 2;
    private static final float Y_OFFSET = (BASE_HEIGHT + HEAD_AND_BALL_SCALE_DIFF) / 16;

    private static final float PULSE_AMPLITUDE = 0.55F;
    ...
    public void render(CrystalBallBlockEntity blockEntity, float tickProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        RenderLayer renderLayer = RenderLayer.getEntityTranslucent(this.playerSkinProvider.getSkinTextures(blockEntity.player.getGameProfile()).texture());
    ...
    private static void transformMatrices(CrystalBallBlockEntity blockEntity, float tickProgress, MatrixStack matrices) {
        matrices.translate(0.5F, Y_OFFSET + getAnimationWobble(blockEntity, tickProgress, 0.15F, 0.01F), 0.5F);

        Direction facing = blockEntity.getCachedState().get(CrystalBallBlock.FACING);

        matrices.multiply(facing.getRotationQuaternion());
        matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(90 + getAnimationWobble(blockEntity, tickProgress, 0.08F, 4.0F)));
        matrices.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(getAnimationWobble(blockEntity, tickProgress, 0.1F, 3.0F)));
    ...
    private static float getAnimationWobble(CrystalBallBlockEntity blockEntity, float tickProgress, float wavelength, float amplitude) {
        return MathHelper.sin((blockEntity.apparitionTickCountdownTime + tickProgress) * wavelength) * amplitude;
    }
    ...
    private static void renderModel(CrystalBallBlockEntity blockEntity, float tickProgress, MatrixStack matrices, VertexConsumer vertexConsumer, int light, SkullEntityModel model) {
        float animProgress = (blockEntity.apparitionTickCountdownTime + tickProgress - MAX_APPARITION_TICKS) / MAX_APPARITION_TICKS;
        float dampenCoefficient = 1 - animProgress;

        float alpha = MathHelper.abs(MathHelper.sin(dampenCoefficient * MathHelper.PI)) * PULSE_AMPLITUDE * dampenCoefficient;
        int color = ColorHelper.Argb.fromFloats(alpha, 0.9F, 0.2F, 1.0F);
